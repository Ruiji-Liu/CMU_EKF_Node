...
#include <utility>
#include <tuple>
#include "dummy_vision/line_2pts.h"
#include "dummy_vision/line_list.h"
...
// USER PARAMETERS
constexpr double initialCovarianceInput[2][2] = {{0.001, 0  },
                                                 {0  , 0.1}};
...
constexpr double initialModelErrorInput[2][2] = {{0.001, 0},
                                                 {0, 0.001}};
constexpr double initialMeasurementErrorInput[2][2] = {{0.001, 0   },
                                                       {0  , 0.001}};

constexpr unsigned maxInitialStateDetections = 5;

constexpr double ROW_SPACING = 1.0;
...
constexpr char modeTopic[] = "/modes";
constexpr char odomTopic[] = "/odometry/filtered";
// constexpr char odomTopic[] = "/odom";
constexpr char switched_lineTopic[] = "/switch_lines";
//constexpr char odomTopic[] = "/odom";
...
std::vector<CMU_EKF_Node::line_polar> pointsToPolar(const dummy_vision::line_list::ConstPtr &msg);
double getYaw(const double w, const double x, const double y, const double z);
void lineCallback(const dummy_vision::line_list::ConstPtr &msg);
...
void modeCallback(const std_msgs::Int32::ConstPtr& msg) {
    mode = msg->data;
    // std::cout << "The mode value is: " << mode << std::endl;
    // return mode;
    if(mode == 2){
        previousmode = mode;
    }
...
void switchlinesCallback(const std_msgs::Int32::ConstPtr& msg) {
    switched_line = msg->data;
    // std::cout << "The mode value is: " << mode << std::endl;
    // return mode;
...
void lineCallback(const dummy_vision::line_list::ConstPtr &msg) {
    // startKalmanFilters();
    // std::cout << "time frame " << time_frame << std::endl;
    // if (time_frame % 10 == 0) {
    //     startKalmanFilters();
    //     time_frame == 0;}
    // time_frame ++;
    // std::cout << "mode right now " << mode << std::endl;
    // std::cout << "previous mode right now " << previousmode << std::endl;
    if(initial_start  == 0){
        startKalmanFilters();
        initial_start++;
    }
    ...
    header_msg.stamp = ros::Time::now(); // Current time
    header_msg.frame_id = "ekf_lines";
    header_msg.seq = sequence_number++;
    linesMsg.Header= header_msg;
    linesMsg.left = left;
...
bool startKalmanFilters() {
    ...
    std::cout << "The parameter value is: " << odomTopic << std::endl;
    ...
    dummy_vision::line_list::ConstPtr list {ros::topic::waitForMessage<dummy_vision::line_list>(lineTopic)};
    ...
    for (int i{0}; i < maxInitialStateDetections && 
        ...
        list = ros::topic::waitForMessage<dummy_vision::line_list>(lineTopic);
    ...
    // std::cout << "Odom " << initialOdom << std::endl;
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber submode = n.subscribe<std_msgs::Int32>(modeTopic, 1, modeCallback);
    ros::Subscriber subswitchlines = n.subscribe<std_msgs::Int32>(switched_lineTopic, 1, switchlinesCallback);
    // if (mode == 1 || mode == 2){
    //     std::cout << "The mode value is: " << mode << std::endl;
    //     return 0;
    // }
    // else{
    // std::cout << "The mode value is: " << submode << std::endl;
    ros::Subscriber subLines = n.subscribe<dummy_vision::line_list>(lineTopic, 1, lineCallback);
